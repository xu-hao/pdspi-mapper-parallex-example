...
def get_patient_resource_entry_array(json_in_dir, pids : List[str], resource_name):
        ...
        for f in os.listdir(resource_path):
            ...
            with open(pid_fn, encoding='latin-1') as pid_fp:
                ...
                if resource_name == "Patient":
                    ...
                    patient_references = [f"Patient/{pid}" for pid in pids]
                    logger.info(f"patient_references = {patient_references}")
                    rescs_filtered.extend(filter(lambda x: x["subject"]["reference"] in patient_references, rescs))
...
def get_entries(json_in_dir, pids : List[str], resource_names):
    ...
    for resource in resource_names:
        entries[resource] = get_patient_resource_entry_array(json_in_dir, pids, resource)
    return entries
